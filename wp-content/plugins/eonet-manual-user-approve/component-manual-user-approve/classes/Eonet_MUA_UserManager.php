...
class Eonet_MUA_UserManager {
	...
	public function __construct($user = null) {
		...
		if(!($user instanceof \WP_User))
			throw new \Exception('Impossible to create an Eonet_MUA_UserManager object. Unkwon data type.');
	...
	public function save_status($status, $alert_user = true){
		...
		do_action('eonet_mua_user_status_updated', $status, $this->user->ID, $alert_user);

		$action_label = '';
		$action_label = ($status == Eonet_MUA_UserManager::APPROVED) ? 'approved' : '';
		$action_label = ($status == Eonet_MUA_UserManager::DENIED) ? 'denied' : '';

		if(!empty($action_label))
			do_action('eonet_mua_user_' . $action_label, $this->user->ID);

		$this->user_status = $status;

		return update_user_meta( $this->user->ID, 'eonet_mua_status', $status );
	}
	...
	public function get_user_status($exact_value = false) {
		...
		$user_status = get_user_meta( $this->user->ID, 'eonet_mua_status', true );
	...
	public function reset_password() {
		...
		//var_dump(defined('EONET_MUA_AVOID_PASSWORD_RESET') && true == EONET_MUA_AVOID_PASSWORD_RESET);
		//var_dump(!empty($this->get_first_access_flag()));
		//var_dump(empty($this->get_user_status(true)));

		//If the password reset has been programmatically removed, don't reset
		$avoid_password_reset = apply_filters('eonet_mua_avoid_password_reset', false);
		...
		$fisrt_access_flag = $this->get_first_access_flag();
		if($fisrt_access_flag == 1)
			return $password;
	...
	public function save_first_access_flag() {
		if ( ! get_user_meta( $this->user->ID, 'eonet_mua_first_access' ) ) {
			add_user_meta( $this->user->ID, 'eonet_mua_first_access', 1 );
		}
	}
	...
	public function get_first_access_flag() {
		return get_user_meta( $this->user->ID, 'eonet_mua_first_access', true );
	}
	...
	public function is_allowed_to_change_users_status() {
		...
		return apply_filters('eonet_mua_is_user_allowed_to_change_status', $user_can, $this->user->ID);
	}
	...
	public static function get_status_label($status){
		...
		if($status == self::APPROVED)
			$label =  __('approved', 'eonet-manual-user-approve');
		if($status == self::PENDING)
			$label = __('pending', 'eonet-manual-user-approve');
		if($status == self::DENIED)
			$label = __('denied', 'eonet-manual-user-approve');
