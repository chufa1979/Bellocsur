...
class T4PSC_PricingTable {
    ...
    public function __construct() {
		...
		add_filter( 't4p_attr_pricingtable-shortcode', array( $this, 'attr' ) );
		add_filter( 't4p_attr_pricingtable-shortcode-column-wrapper', array( $this, 'column_wrapper_attr' ) );	
		add_filter( 't4p_attr_pricingtable-shortcode-price', array( $this, 'price_attr' ) );
		add_filter( 't4p_attr_pricingtable-shortcode-row', array( $this, 'row_attr' ) );
		add_filter( 't4p_attr_pricingtable-shortcode-footer', array( $this, 'footer_attr' ) );
    ...
    function render_parent( $args, $content = '') {
		...
		$defaults = T4PCore_Plugin::set_shortcode_defaults(
			array(
				'class' 			=> '',
				'id' 				=> '',
				'backgroundcolor' 	=> $smof_data['pricing_bg_color'].$evl_options['evl_shortcode_pricing_box_bg_color'],
				'bordercolor' 		=> $smof_data['pricing_border_color'].$evl_options['evl_shortcode_pricing_box_border_color'],
				'columns'			=> '',
				'dividercolor' 		=> $smof_data['pricing_divider_color'].$evl_options['evl_shortcode_pricing_box_divider_color'],
				'type'				=> '1',
			), $args
		);
		...
		$html = sprintf( '%s<div %s>%s</div>', $styles, T4PCore_Plugin::attributes( 'pricingtable-shortcode' ), do_shortcode($content) );
	...
	function attr() {
        ...
        $attr['class'] = sprintf( 't4p-pricing-table pricing-table-%s %s-boxed-pricing row t4p-columns-%s columns-%s t4p-clearfix', 
    ...
    function render_child_column( $args, $content = '') {
		...
		$html = sprintf( '<div %s><div %s><div %s><div %s><h3 %s>%s</h3></div>%s', T4PCore_Plugin::attributes( 'pricingtable-shortcode-column-wrapper' ), 
						 T4PCore_Plugin::attributes( 'panel-container' ), T4PCore_Plugin::attributes( 'panel' ), T4PCore_Plugin::attributes( 'panel-heading' ), 
						 T4PCore_Plugin::attributes( 'title-row' ), $title, do_shortcode( $content ) );
	...
	function column_wrapper_attr() {
		...
		$attr['class'] = 'panel-wrapper t4p-column column col-lg-' . $columns . ' col-md-' . $columns . ' col-sm-' . $columns;
    ...
    function render_child_price( $args, $content = '') {
		...
		if( isset( $currency ) && ! empty( $currency ) && isset( $price ) && ( ! empty( $price ) || ( $price == '0') ) ) {
			...
			if( $currency_position != 'right' ){
				$pricing = sprintf( '<span %s>%s</span>', T4PCore_Plugin::attributes( 'currency' ), $currency );
			}
			
			$pricing .= sprintf( '<span %s>%s</span>', T4PCore_Plugin::attributes( 'integer-part' ), $price[0] );			
			
			if( $price[1] ){
				$pricing .= sprintf( '<sup %s>%s</sup>', T4PCore_Plugin::attributes( 'decimal-part' ), $price[1] );
			}
			
			if( $currency_position == 'right' ){
				$pricing .= sprintf( '<span %s>%s</span>', T4PCore_Plugin::attributes( 'currency pos-right' ), $currency );
				
				if( $time ) {
					$pricing .= sprintf( '<br /><span %s>%s</span>', T4PCore_Plugin::attributes( 'time pos-right' ), $time );
				}				
			}			
			...
			if( $time &&
				...
				$pricing .= sprintf( '<br /><span %s>%s</span>', T4PCore_Plugin::attributes( 'time' ), $time );
			}			
		
			$html = sprintf( '<div %s><div %s>%s</div></div>%s', T4PCore_Plugin::attributes( 'panel-body pricing-row' ), T4PCore_Plugin::attributes( 'price ' . $pricing_class ), 
    ...
    function render_child_row( $args, $content = '') {
		...
		if( $this->is_first_row ) {
			$html = sprintf( '<ul %s>', T4PCore_Plugin::attributes( 'list-group' ) );
			$this->is_first_row = false;
		}	
		
		$html .= sprintf( '<li %s>%s</li>', T4PCore_Plugin::attributes( 'list-group-item normal-row' ), do_shortcode( $content ) );
