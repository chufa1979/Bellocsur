...
class T4PSC_FlipBoxes {
	...
	public function __construct() {
		...
		add_filter( 't4p_attr_flip-boxes-shortcode', array( $this, 'parent_attr' ) );
		add_shortcode( 'flip_boxes', array( $this, 'render_parent' ) );

		add_filter( 't4p_attr_flip-box-shortcode', array( $this, 'child_attr' ) );
		add_filter( 't4p_attr_flip-box-shortcode-front-box', array( $this, 'front_box_attr' ) );
		add_filter( 't4p_attr_flip-box-shortcode-back-box', array( $this, 'back_box_attr' ) );
		add_filter( 't4p_attr_flip-box-shortcode-heading-front', array( $this, 'heading_front_attr' ) );
		add_filter( 't4p_attr_flip-box-shortcode-heading-back', array( $this, 'heading_back_attr' ) );
		add_filter( 't4p_attr_flip-box-shortcode-grafix', array( $this, 'grafix_attr' ) );		
		add_filter( 't4p_attr_flip-box-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<div %s>%s</div><div %s></div>', T4PCore_Plugin::attributes( 'flip-boxes-shortcode' ), do_shortcode( $content ), T4PCore_Plugin::attributes( '' ) );
	...
	function parent_attr() {
		...
		$attr['class'] = 't4p-flip-boxes flip-boxes';
	...
	function render_child( $args, $content = '') {
		...
		$defaults =	T4PCore_Plugin::set_shortcode_defaults(
			array(
				'class'						=> '',
				'id'						=> '',
				'background_color_front'	=> $evl_options['evl_shortcode_flip_boxes_bg_color_frontside'].$smof_data['flip_boxes_front_bg'],
				'background_color_back' 	=> $evl_options['evl_shortcode_flip_bg_color_backside'].$smof_data['flip_boxes_back_bg'],
				'border_color'				=> $evl_options['evl_shortcode_flip_border_color'].$smof_data['flip_boxes_border_color'],
				'border_radius'				=> $evl_options['evl_shortcode_flip_border_radius'].$smof_data['flip_boxes_border_radius'],
				'border_size'				=> $evl_options['evl_shortcode_flip_border_size'].$smof_data['flip_boxes_border_size'],
				'circle'					=> '',
				'circle_color' 				=> $evl_options['evl_shortcode_icon_circle_color'].$smof_data['icon_circle_color'],
				'circle_border_color' 		=> $evl_options['evl_shortcode_icon_circle_border_color'].$smof_data['icon_border_color'],
				'icon' 						=> '',
				'icon_color' 				=> $evl_options['evl_shortcode_icon_color'].$smof_data['icon_color'],
				'icon_flip'					=> '',
				'icon_rotate'				=> '',
				'icon_spin'					=> '',				
				'image' 					=> '',
				'image_width' 				=> '35',
				'image_height' 				=> '35',
				'text_back_color'			=> $evl_options['evl_shortcode_flip_text_color_backside'].$smof_data['flip_boxes_back_text'],
				'text_front'				=> '',
				'text_front_color'			=> $evl_options['evl_shortcode_flip_text_color_frontside'].$smof_data['flip_boxes_front_text'],
				'title_front' 				=> '',
				'title_front_color' 		=> $evl_options['evl_shortcode_flip_boxes_heading_color_frontside'].$smof_data['flip_boxes_front_heading'],
				'title_back' 				=> '',
				'title_back_color' 			=> $evl_options['evl_shortcode_flip_heading_color_backside'].$smof_data['flip_boxes_back_heading'],
				'animation_type' 			=> '',
	...
	function child_attr() {
		...
		$attr['class'] = sprintf('t4p-flip-box-wrapper col-lg-%s col-md-%s col-sm-%s', $columns, $columns, $columns );
	...
	function icon_attr() {
		...
		if( self::$child_args['image'] ) {
			...
			$attr['class'] = sprintf( 'fa %s', T4PCore_Plugin::font_awesome_name_handler( self::$child_args['icon'] ) );
		}
