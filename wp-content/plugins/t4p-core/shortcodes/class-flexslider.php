...
class T4PSC_Flexslider {
	...
	public function __construct() {
		...
		add_filter( 't4p_attr_flexslider-shortcode', array( $this, 'attr' ) );
		add_filter( 't4p_attr_flexslider-shortcode-slides-container', array( $this, 'slides_container_attr' ) );
		add_filter( 't4p_attr_flexslider-shortcode-caption', array( $this, 'caption_attr' ) );
		add_filter( 't4p_attr_flexslider-shortcode-title-container', array( $this, 'title_container_attr' ) );
		add_filter( 't4p_attr_flexslider-shortcode-thumbnails', array( $this, 'thumbnails_attr' ) );
			...
			if( $query->have_posts() ):
				...
				while( $query->have_posts() ): $query->the_post();
					...
					if( isset( $meta['caption'] ) && $meta['caption'] ) {
						$caption = sprintf( '<p %s>%s</p>', T4PCore_Plugin::attributes( 'flexslider-shortcode-caption' ), $meta['caption'] );
					}

					$html .= sprintf( '<li>%s</li>', t4p_content( self::$args['excerpt'], true ) . $caption );
	...
	function posts_excerpt() {
		...
		if( $query->have_posts() ):
			...
			while( $query->have_posts() ): $query->the_post();
				...
				$container = sprintf( '<div %s><div %s>%s</div></div>', T4PCore_Plugin::attributes( 'flexslider-shortcode-title-container' ), 
									  T4PCore_Plugin::attributes( 'excerpt-container' ), $title . $excerpt );
	...
	function attr() {
		...
		$attr['class'] = 't4p-flexslider flexslider flexslider-' . self::$args['layout'];
