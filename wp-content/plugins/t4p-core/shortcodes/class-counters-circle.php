...
class T4PSC_CountersCircle {
	...
	public function __construct() {
		...
		add_filter( 't4p_attr_counters-circle-shortcode', array( $this, 'parent_attr' ) );
		add_shortcode( 'counters_circle', array( $this, 'render_parent' ) );

		add_filter( 't4p_attr_counter-circle-shortcode', array( $this, 'child_attr' ) );
		add_filter( 't4p_attr_counter-circle-wrapper-shortcode', array( $this, 'child_wrapper_attr' ) );
	...
	function render_parent( $args, $content = '' ) {
		...
		$html = sprintf( '<div %s>%s</div>', T4PCore_Plugin::attributes( 'counters-circle-shortcode' ), do_shortcode( $content ) );
	...
	function parent_attr() {
		...
		$attr['class'] = 't4p-counters-circle counters-circle';
	...
	function render_child( $args, $content = '' ) {
		...
		$defaults =	T4PCore_Plugin::set_shortcode_defaults(
			array(
				...
				'id'			=> '',
				'countdown'		=> 'no',
				'filledcolor' 	=> strtolower( $smof_data['counter_filled_color'] ).strtolower( $evl_options['evl_shortcode_counter_circle_filled_color'] ),
				'unfilledcolor' => strtolower( $smof_data['counter_unfilled_color'] ).strtolower( $evl_options['evl_shortcode_counter_circle_unfilled_color'] ),
				'scales'		=> 'no',
				'size' 			=> '220',
				'speed' 		=> '1500',
				'value' 		=> '1',				
	...
	function child_attr() {
		...
		$attr['class'] = 't4p-counter-circle counter-circle counter-circle-content';
