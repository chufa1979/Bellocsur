...
class T4PSC_Button {
    ...
    function render($args, $content = '') {
        ...
        if( self::$args['align'] == 'center' ) {
                $html = "<div class='button-align-center'>$html</div>";
        }
    ...
    function attr() {
        ...
        if ( strtolower( self::$args['align'] ) == 'left' )
                $cls_alignment = 'pull-left';
        if ( strtolower( self::$args['align'] ) == 'right' )
                $cls_alignment = 'pull-right';

        $attr['class'] = sprintf('btn-default %s button %3$s button %4$s t4p-button button-%s button-%s button-%s button-%s button-%s buttonshadow-%s', $cls_alignment, self::$args['type'], self::$args['shape'], self::$args['size'], self::$args['color'], $this->button_counter, self::$args['shadow']);
