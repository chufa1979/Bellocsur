...
class T4PSC_Person {
    ...
    public function __construct() {
        ...
        add_filter( 't4p_attr_person-shortcode', array( $this, 'attr' ) );
        add_filter( 't4p_attr_person-shortcode-image-container', array( $this, 'image_container_attr' ) );
		    add_filter( 't4p_attr_person-shortcode-href', array( $this, 'href_attr' ) );
        add_filter( 't4p_attr_person-shortcode-img', array( $this, 'img_attr' ) );
        add_filter( 't4p_attr_person-shortcode-author', array( $this, 'author_attr' ) );
        add_filter( 't4p_attr_person-shortcode-social-networks', array( $this, 'social_networks_attr' ) );	
        add_filter( 't4p_attr_person-shortcode-icon', array( $this, 'icon_attr' ) );	
    ...
    function render( $args, $content = '') {
        ...
        $defaults = T4PCore_Plugin::set_shortcode_defaults(
        	array(
				...
				'linktarget'				=> '_self',
				'name'						=> '',
				'social_icon_boxed'			=> strtolower( $evl_options['evl_shortcode_social_icon_boxed'] ).strtolower( $smof_data['social_links_boxed'] ),
				'social_icon_boxed_colors'	=> strtolower( $evl_options['evl_shortcode_social_icon_box_color'] ).strtolower( $smof_data['social_links_box_color'] ),					
				'social_icon_boxed_radius' 	=> strtolower( $evl_options['evl_shortcode_social_icon_box_radius'] ).strtolower( $smof_data['social_links_boxed_radius'] ),
				'social_icon_colors'		=> strtolower( $evl_options['evl_shortcode_social_icon_boxed'] ).strtolower( $smof_data['social_links_icon_color'] ),			
				'social_icon_order'			=> '',
				'social_icon_tooltip'		=> strtolower( $evl_options['evl_shortcode_social_icon_tooltip_position'] ).strtolower( $smof_data['social_links_tooltip_placement'] ),
				'pic_bordercolor'			=> strtolower( $evl_options['evl_shortcode_person_border_color'] ).strtolower( $smof_data['person_border_color'] ),				
				'pic_bordersize'			=> strtolower( $evl_options['evl_shortcode_person_border_size'] ).strtolower( $smof_data['person_border_size'] ),
				'pic_link'					=> '',
				'pic_style'					=> 'none',
				'pic_style_color'			=> strtolower( $evl_options['evl_shortcode_person_style_color'] ).strtolower( $smof_data['person_style_color'] ),
				'picture'					=> '',
				'title'						=> '',
				
				'facebook' => '', 'twitter' => '', 'linkedin' => '', 'dribbble' => '', 'rss' => '', 'youtube' => '', 'pinterest' => '', 'flickr' => '', 'vimeo' => '', 
		...
		if( $picture ) {
			...
			$picture =  sprintf( '<div %s><div %s>%s</div></div>', T4PCore_Plugin::attributes( 'person-shortcode-image-wrapper' ), 
								 T4PCore_Plugin::attributes( 'person-shortcode-image-container' ), $picture ) ;
		...
		if( $name || $title || $content ) {
			...
			$inner_content .= sprintf( '<div %s><div %s><div %s><span %s>%s</span><span %s>%s</span></div><div %s>%s</div></div><div %s>%s</div></div>', 
									   T4PCore_Plugin::attributes( 'person-desc' ), T4PCore_Plugin::attributes( 'person-shortcode-author' ), 
									   T4PCore_Plugin::attributes( 'person-author-wrapper' ), T4PCore_Plugin::attributes( 'person-name' ), $name,
									   T4PCore_Plugin::attributes( 'person-title' ), $title,							  		   
							  		 T4PCore_Plugin::attributes( 'person-content t4p-clearfix' ), do_shortcode( $content ),	
                     T4PCore_Plugin::attributes( 'person-shortcode-social-networks' ), $icons );			
