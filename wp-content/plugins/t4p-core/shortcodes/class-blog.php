...
class T4PSC_Blog {
    ...
    public function __construct() {
        ...
        // containers
        add_action('t4p_blog_shortcode_before_loop', array($this, 'before_loop'));
        add_action('t4p_blog_shortcode_before_loop_timeline', array($this, 'before_loop_timeline'));
        add_action('t4p_blog_shortcode_after_loop', array($this, 'after_loop'));

        // post / loop basic structure
        add_action('t4p_blog_shortcode_loop_header', array($this, 'loop_header'));
        add_action('t4p_blog_shortcode_loop_footer', array($this, 'loop_footer'));
        add_action('t4p_blog_shortcode_loop_content', array($this, 'loop_content'));
        add_action('t4p_blog_shortcode_loop_content', array($this, 'page_links'));
        add_action('t4p_blog_shortcode_loop', array($this, 'loop'));

        // special blog layout structure
        add_action('t4p_blog_shortcode_wrap_loop_open', array($this, 'wrap_loop_open'));
        add_action('t4p_blog_shortcode_wrap_loop_close', array($this, 'wrap_loop_close'));
        add_action('t4p_blog_shortcode_date_and_format', array($this, 'date_and_format'));
        add_action('t4p_blog_shortcode_timeline_date', array($this, 'timeline_date'));
        add_action('t4p_blog_shortcode_entry_meta_alternate', array($this, 'entry_meta_alternate'));
        add_action('t4p_blog_shortcode_entry_meta_grid_timeline', array($this, 'entry_meta_grid_timeline'));

        // loop footer content
        add_action('t4p_blog_shortcode_entry_meta_default', array($this, 'entry_meta_default'));

        // element attributes
        add_filter('t4p_attr_blog-shortcode', array($this, 'attr'));
        add_filter('t4p_attr_blog-shortcode-posts-container', array($this, 'posts_container_attr'));


        add_filter('t4p_attr_blog-shortcode-loop', array($this, 'loop_attr'));
        add_filter('t4p_attr_blog-shortcode-post-title', array($this, 'post_title_attr'));
        add_filter('t4p_attr_blog-shortcode-entry-meta', array($this, 'entry_meta_attr'));
        add_filter('t4p_attr_blog-shortcode-entry-meta-details', array($this, 'entry_meta_details_attr'));
        add_filter('t4p_attr_blog-shortcode-meta-author', array($this, 'meta_author_attr'));
        add_filter('t4p_attr_blog-shortcode-meta-author-link', array($this, 'meta_author_link_attr'));
        add_filter('t4p_attr_blog-shortcode-meta-date', array($this, 'meta_date_attr'));
        add_filter('t4p_attr_blog-shortcode-meta-date-updated', array($this, 'meta_date_updated_attr'));
        add_filter('t4p_attr_blog-shortcode-meta-tags', array($this, 'meta_tags_attr'));
    ...
    function render($args, $content = '') {
        ...
        $t4p_query = new WP_Query($defaults);
        ...
        $html .= sprintf('<div %s>', T4PCore_Plugin::attributes('blog-shortcode'));

        $html .= sprintf('<div %s>', T4PCore_Plugin::attributes('blog-shortcode-posts-container'));

        ob_start();
        do_action('t4p_blog_shortcode_wrap_loop_open');
        ...
        if ($t4p_query->have_posts()) : while ($t4p_query->have_posts()) : $t4p_query->the_post();
                ...
                if (self::$args['layout'] == 'timeline') {
                    ...
                    ob_start();
                    do_action('t4p_blog_shortcode_timeline_date', $date_params);

                    do_action('t4p_blog_shortcode_before_loop_timeline');
                    ...
                    ob_start();
                    do_action('t4p_blog_shortcode_before_loop');
                ...
                if (self::$args['layout'] == 'grid' ||
                    ...
                    $html .= sprintf('<div %s>', T4PCore_Plugin::attributes('post-content-wrapper'));
                }
                ...
                ob_start();
                do_action('t4p_blog_shortcode_loop_header');

                do_action('t4p_blog_shortcode_loop_content');

                do_action('t4p_blog_shortcode_loop_footer');

                do_action('t4p_blog_shortcode_after_loop');
        ...
        ob_start();
        do_action('t4p_blog_shortcode_wrap_loop_close');
    ...
    function attr() {
        ...
        $attr['class'] = sprintf('t4p-blog-shortcode t4p-blog-%s t4p-blog-%s', $blog_layout, self::$args['scrolling']);
    ...
    function posts_container_attr() {
        ...
        $attr['class'] = sprintf('t4p-posts-container posts-container-%s', self::$args['scrolling']);
        if (self::$args['layout'] == 'grid') {
    ...
    function wrap_loop_open() {
        ...
        if (self::$args['layout'] == 'timeline') {
            ...
            $wrapper .= sprintf('<div %s>', T4PCore_Plugin::attributes('blog-timeline-layout'));
        }
...
// end wrap_loop_close()
    ...
    function before_loop() {
        ...
        echo sprintf('<div %s>', T4PCore_Plugin::attributes('blog-shortcode-loop')) . "\n";
    }
...
// end after_loop()
    ...
    function loop_attr() {
        ...
        if (self::$args['layout'] == 'timeline') {
            ...
            $extra_classes[] = 'blog-timeline t4p-clearfix' . $formatted_class;
        ...
        if ($post_class && is_array($post_class)) {
            ...
            $attr['class'] = $classes . $formatted_class;
        }
    ...
    function get_slideshow() {
        ...
        if (self::$args['layout'] == 'medium' ||
            ...
            if (self::$args['layout'] == 'medium_alternate') {
                ob_start();
                do_action('t4p_blog_shortcode_date_and_format');
            ...
            $html .= sprintf('<div %s>', T4PCore_Plugin::attributes('blog-medium-slideshow-container'));
...
// end get_post_thumbnails()
    ...
    function loop_header() {
        ...
        if (self::$args['layout'] == 'large' || self::$args['layout'] == 'large alternate') {
            ob_start();
            do_action('t4p_blog_shortcode_entry_meta_alternate');
        ...
        if (self::$args['layout'] == 'grid' ||
            ...
            if ((!self::$args['meta_all'] && self::$args['excerpt_words'] == '0' ) ||
                ...
                $content_sep = sprintf('<div %s></div>', T4PCore_Plugin::attributes('no-content-sep'));
            } else {
                $content_sep = sprintf('<div %s></div>', T4PCore_Plugin::attributes('content-sep'));
            }

            if (self::$args['meta_all']) {
                ob_start();
                do_action('t4p_blog_shortcode_entry_meta_grid_timeline');
                $meta_data = ob_get_contents();
                ob_get_clean();
            }
        }

        $pre_title_content .= sprintf('<div %s>', T4PCore_Plugin::attributes('post-content-container'));
        ...
        if (self::$args['layout'] == 'timeline') {
            $pre_title_content .= sprintf('<div %s></div><div %s></div>', T4PCore_Plugin::attributes('timeline-circle'), T4PCore_Plugin::attributes('timeline-arrow'));
        }

        $html = sprintf('%s<h2 %s>%s</h2>%s%s', $pre_title_content, T4PCore_Plugin::attributes('blog-shortcode-post-title'), $link, $meta_data, $content_sep);
...
// end post_title_attr();
    ...
    function loop_footer() {
        ...
        if (self::$args['meta_all'] && (self::$args['layout'] == 'medium' || self::$args['layout'] == 'medium alternate')) {
            do_action('t4p_blog_shortcode_entry_meta_alternate');
        }
        ...
        if (self::$args['meta_all'] && (self::$args['layout'] == 'medium' ||
            ...
            do_action('t4p_blog_shortcode_entry_meta_default');
        }
...
// end loop_footer()
    ...
    function date_and_format() {
        ...
        $inner_content = sprintf('<div %s>', T4PCore_Plugin::attributes('date-and-formats'));
        $inner_content .= sprintf('<div %s>', T4PCore_Plugin::attributes('date-box updated'));
        ...
        $inner_content .= sprintf('</div>
									...
									</div>', T4PCore_Plugin::attributes('format-box'), T4PCore_Plugin::attributes('t4p-icon-' . $format_class));
...
// end add_date_and_format()
    ...
    function timeline_date($date_params) {
        ...
        if ($args['prev_post_month'] != $args['post_month']) {
            $inner_content = sprintf('<div %s><h3 %s style="font-size:13px !important; padding: 0px 5px;">%s</h3></div>', T4PCore_Plugin::attributes('timeline-date hidden-div'), T4PCore_Plugin::attributes('timeline-title'), get_the_date($smof_data['timeline_date_format']));
        }
        ...
        if ('evolve Plus' == $theme->name || 'evolve Plus' == $theme->parent_theme) {
            if (self::$args['layout'] == 'large') {
                if (self::$args['meta_categories']) {
                    ...
                    $inner_content .= sprintf('<span %s>%s</span><span %s>|</span>', T4PCore_Plugin::attributes('entry-categories'), $output, T4PCore_Plugin::attributes('meta-separator'));
                }
				if (self::$args['meta_tags']) {
                    $inner_content .= sprintf('%s<span %s>|</span>', $this->post_meta_tags(), T4PCore_Plugin::attributes('meta-separator'));
                }
            }
        }

        $entry_meta = sprintf('<div %s></div><div %s>%s</div>', T4PCore_Plugin::attributes('t4p-clearfix'), T4PCore_Plugin::attributes('blog-shortcode-entry-meta'), $inner_content);
...
// end entry_meta_default()
    ...
    function entry_meta_alternate() {
        ...
        $entry_meta = sprintf('<div %s>%s</div>', T4PCore_Plugin::attributes('blog-shortcode-entry-meta'), $inner_content);
...
// end entry_meta_grid_timeline()
    ...
    function post_meta_data($return_all_meta = false) {
        ...
        $inner_content = sprintf('<p %s>', T4PCore_Plugin::attributes('blog-shortcode-entry-meta-details'));

        $theme = wp_get_theme(); // gets the current theme
        if ('evolve Plus' == $theme->name || 'evolve Plus' == $theme->parent_theme) {
            if (self::$args['meta_date']) {
                $inner_content .= sprintf('<span %s><span %s>%s</span><time %s>%s</time></span><span %s>|</span>', T4PCore_Plugin::attributes('entry-time'), T4PCore_Plugin::attributes('blog-shortcode-meta-date-updated'), get_the_modified_time('c'), T4PCore_Plugin::attributes('blog-shortcode-meta-date'), get_the_time(' F j, Y'), T4PCore_Plugin::attributes('meta-separator'));
            }

            if (self::$args['meta_author']) {
                $inner_content .= sprintf('<span %s>' . __('Written By', 't4p-core') . ' <a %s>%s</a>' . '</span><span %s>|</span>', T4PCore_Plugin::attributes('blog-shortcode-meta-author'), T4PCore_Plugin::attributes('blog-shortcode-meta-author-link'), get_the_author_meta('display_name'), T4PCore_Plugin::attributes('meta-separator'));
            }
			...
			if (self::$args['meta_comments']) {
                ...
                $inner_content .= sprintf('<span %s>%s</span><span %s>|</span>', T4PCore_Plugin::attributes('entry-comments'), $comments, T4PCore_Plugin::attributes('meta-separator'));
            }
			}
        } else {
            if (self::$args['meta_author']) {
                $inner_content .= sprintf('<span %s>' . __('By', 't4p-core') . ' <a %s>%s</a>' . '</span><span %s>|</span>', T4PCore_Plugin::attributes('blog-shortcode-meta-author'), T4PCore_Plugin::attributes('blog-shortcode-meta-author-link'), get_the_author_meta('display_name'), T4PCore_Plugin::attributes('meta-separator'));
            }

            if (self::$args['meta_date']) {
                $inner_content .= sprintf('<span %s><span %s>%s</span><time %s>%s</time></span><span %s>|</span>', T4PCore_Plugin::attributes('entry-time'), T4PCore_Plugin::attributes('blog-shortcode-meta-date-updated'), get_the_modified_time('c'), T4PCore_Plugin::attributes('blog-shortcode-meta-date'), get_the_time($smof_data['date_format']), T4PCore_Plugin::attributes('meta-separator'));
            }
        }
...
// end post_meta_data()
    ...
    function grid_timeline_comments() {
        ...
        if (self::$args['meta_comments']) {
            ...
            $comments_icon = sprintf('<i %s></i>&nbsp;', T4PCore_Plugin::attributes('t4p-icon-comment'));
            ob_start();
            comments_popup_link($comments_icon . __('0', 't4p-core'), $comments_icon . __('1', 't4p-core'), $comments_icon . __('%', 't4p-core'));
            $comments = ob_get_contents();
            ob_get_clean();

            $inner_content = sprintf('<span %s>%s</span>', T4PCore_Plugin::attributes('comment-number'), $comments);
...
// end grid_timeline_comments()
    ...
    function post_meta_tags() {
        ...
        if (self::$args['meta_tags']) {
            ...
            $inner_content = sprintf('<span %s>%s</span>', T4PCore_Plugin::attributes('blog-shortcode-meta-tags'), $tags);
        }
    ...
    function read_more() {
        ...
        if (self::$args['meta_link']) {
            ...
            if (self::$args['meta_read']) {
                ...
                $inner_content .= sprintf('<p %s>', T4PCore_Plugin::attributes('entry-read-more'));
                $inner_content .= sprintf('<a %s href="%s">%s</a>', T4PCore_Plugin::attributes('read-more btn t4p-button-default'), get_permalink(), __('Read More', 't4p-core'));
                $inner_content .= '</p>';
            }
