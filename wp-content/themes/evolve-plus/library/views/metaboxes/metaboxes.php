...
class evolve_ThemeFrameworkMetaboxes {
    ...
    public function evolve_add_meta_boxes() {
        ...
        $disallowed = array('page', 'post', 'attachment', 'evolve_portfolio', 'theme4press_elastic', 'product', 'wpsc-product', 'slide');
        ...
        $this->evolve_add_meta_box('evolve_portfolio_options', 'Portfolio Options', 'evolve_portfolio');
        $this->evolve_add_meta_box('evolve_woocommerce_options', 'Product Options', 'product');
        $this->evolve_add_meta_box('slide_options', 'Slide Options', 'slide');
    }
    ...
    public function evolve_post_options() {
        get_template_part('library/functions/css/style.min');
        $this->evolve_render_option_tabs(array('layout', 'pagetitlebar', 'widget', 'slider', 'sidebars'));
    }

    public function evolve_page_options() {
        get_template_part('library/functions/css/style.min');
        $this->evolve_render_option_tabs(array('layout', 'pagetitlebar', 'widget', 'slider', 'sidebars', 'pageportfolio'));
    }

    public function evolve_portfolio_options() {
        get_template_part('library/functions/css/style.min');
        $this->evolve_render_option_tabs(array('portfoliopost', 'widget', 'slider', 'sidebars'));
    }
    
    public function evolve_woocommerce_options() {
        get_template_part('library/functions/css/style.min');
        include 'woocommerce_options.php';
    }

    public function slide_options() {
        get_template_part('library/functions/css/style.min');
        include 'slide_options.php';
    }
    ...
    public function evolve_upload($id, $label, $desc = '') {
        ...
        $evolve_upload_src = wp_get_attachment_url($evolve_upload_img_id);
        ...
        if ($evolve_upload_src) :
            $html .= '<input type="text" class="upload_field" value="' . $evolve_upload_src . '" /></br>';
            if ( $id != 'webm' && $id != 'mp4' && $id != 'ogv' ) 
                $html .= '<img src="' . $evolve_upload_src . '" class="redux-option-image" style="width:60px; height:60px;" />';
        endif;
        $html .= '</div>';
    ...
    public function evolve_render_option_tabs($requested_tabs, $post_type = 'default') {
        $tabs_names = array(
            ...
            'pageportfolio' => __('Portfolio', 'evolve'),
            'slider' => __('Slider', 'evolve'),
            'portfoliopost' => __('Portfolio', 'evolve'),
        ...
        $tabs_icons = array(
            ...
            'pageportfolio' => 't4p-icon-appbarimagemultiple',
            'slider' => 't4p-icon-appbarmonitor',
            'portfoliopost' => 't4p-icon-appbarimagemultiple',
