 ...
 */
function t4p_get_adjacent_post_plus($r, $previous = true) {
    global $post, $wpdb;
 ...
 */
function t4p_previous_post_link_plus($args = '') {
    return t4p_adjacent_post_link_plus($args, '&laquo; %link', true);
}
 ...
 */
function t4p_next_post_link_plus($args = '') {
    return t4p_adjacent_post_link_plus($args, '%link &raquo;', false);
}
 ...
 */
function t4p_adjacent_post_link_plus($args = '', $format = '%link &raquo;', $previous = true) {
    $defaults = array(
...
//	If Post Types Order plugin is installed, default to sorting on menu_order
    ...
    if ($previous && is_attachment()) {
        ...
        $posts = t4p_get_adjacent_post_plus($r, $previous);
...
//	When num_results > 1, multiple adjacent posts may be returned. Use foreach to display each adjacent post.
    foreach ($posts as $post) {
        ...
        if (empty($post->post_title))
            $title = $previous ? __('Previous Post', 'evolve') : __('Next Post', 'evolve');
