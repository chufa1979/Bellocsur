...
jQuery(document).ready(function ($) {
    $('.convertsc').on('click', function () {
        var pageid = $( "#evl-front-page-add-page-select option:selected" ).val();

        if ( pageid == '' ) {
            alert ( "You have not select any page" );
            return false;
        }

        if ( !confirm ( "Are you sure you wish to convert the elements in shortcodes?" ) ) {
            return false;
        }
	...
	jQuery.ajax({
		method: 'POST',
		type: 'POST',
		url: evolveajax.ajaxurl,
		data: {
			action : 'evolve_element_to_shortcode',
			evolve_pageid : pageid,
		},
	}).done(function( response ) {
                $('.ajax_spinner').css('display', 'none');
                $( '#evl_options-evl-front-page-shortcode .status_msg' ).html( response['status'] );
                $(".convertsc_succ_msg").fadeOut(5000);
